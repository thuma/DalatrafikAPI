...
if(is_file($namelist) == FALSE)
	...
	for($i = 0; $i < count($bokstaver); $i++)
		...
		while($newdata)
			...
			$newdata = FALSE;
			$doc = new DOMDocument();
			@$doc->loadHTML(file_get_contents('http://193.45.213.123/dalarna/v2/indexes.aspx?optType=0&selKommun=0&sLetter='.urlencode(utf8_decode($bokstaver[$i])).'&iPage='.$page.'&Language=se&optFrTo=0&TNSource=DALARNA'));
...
if(is_file($idlist) == FALSE)
	...
	foreach($all as $key => $name)
		{
		$url = 'http://193.45.213.123/dalarna/v2/rpajax.aspx?net=DALARNA&lang=se&letters='.urlencode(utf8_decode($name->cleanname));
...
foreach($all as $key => $station)
	...
	if(isset($station->position)==FALSE)
		{
		//open connection
		$ch = curl_init('http://193.45.213.123/dalarna/v2/querypage_adv.aspx');
		...
		'&optTypeFr=0&inpPointTo=&optTypeTo=0&selDirection=0&inpTime=00%3A26&inpDate=2013-12-08&optReturn=0&selDirection2=0&inpTime2=10%3A57&inpDate2=2013-12-08&trafficmask=1&trafficmask=2&trafficmask=4&trafficmask=8&selChangeTime=0&selWalkSpeed=0&selPriority=0&cmdAction=pastefrommap&EU_Spirit=False&TNSource=DALARNA&SupportsScript=True&Language=se&VerNo=7.1.1.2.0.38p3'.
